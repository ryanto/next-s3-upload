...
The configuration values used here will be different for everyone. Please refer to your providers documentation for the correct values.
...
The `usePresignedUpload()` hook has [the same API](/use-s3-upload) as the `useS3Upload()` hook, the two can be used interchangeably.
...
- Most providers do not support the full S3 API. Certain APIs or function calls may not work simply because your provider does not support them. You should always check with your host about what APIs and features they support.
