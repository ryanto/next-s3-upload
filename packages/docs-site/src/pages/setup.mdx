...
# Optional Custom Endpoint to allow using S3 providers other than AWS (ie: DigitalOcean, Scaleway, Wasabi...)
# To Use: uncomment and update to your own endpoint:
# S3_CUSTOM_ENDPOINT=https://s3.nl-ams.scw.cloud
```
...
The example above only allows `jpg`, `png`, and `gif` files to be uploaded. If you're planning to use `next/image` with files uploaded to S3 you'll want to make sure that only images are allowed to be uploaded. See [this issue](https://github.com/ryanto/next-s3-upload/issues/19) for more information.
